...
export default async function handler(req, res) {
  try {
    // 최근 7일 digest에서 report_texts만 가져오기 (raw_messages 제외 — 속도 핵심)
    ...
    // 별도: 최근 3개 digest에서 raw_messages의 레포트 키워드만 빠르게
    const { data: msgDigests } = await supabase
      .from('telegram_digests')
      .select('raw_messages, date_key')
      .order("collected_at", { ascending: false })
      .limit(2);

    if (error || (!digests?.length && !msgDigests?.length))
    ...
    // 2. 메시지에서 레포트 관련 내용만 (최근 2개 digest에서만)
    const REPORT_KEYWORDS = ['리포트', '리서치', '레포트', '보고서', '목표가', '투자의견', 'BUY', 'SELL', '매수', '매도', '비중확대', '상향', '하향', 'TP ', '영업이익', '컨센서스', '실적발표', '.pdf'];
      ...
      for (const [handle, msgs] of Object.entries(d.raw_messages || {})) {
        for (const msg of msgs) {
          if (msg.text && msg.text.length > 30 && REPORT_KEYWORDS.some(kw => msg.text.includes(kw))) {
            reportMessages.push(`[${d.date_key || ''}][${handle}] ${msg.text.slice(0, 400)}`);
          }
        }
      }
    }
    ...
    const content = [
      ...
      ...allReports.slice(0, 5).map(r => `[파일: ${r.fileName}]\n${r.text.slice(0, 1000)}`),
      reportMessages.length > 0 ? `\n=== 레포트 관련 메시지 (${reportMessages.length}개) ===` : '',
      ...reportMessages.slice(0, 20),
    ].filter(Boolean).join('\n---\n').slice(0, 15000);
